...
6. **未注册端点**（属第 6 片遗留/后续切片，UI 一律禁用并标注原因，不给假入口）：
   ...
   以及 Skill 的 spec 形态。
   【KM-29 同步】`POST /machines/{name}/ssh/probe`、`POST /machines/{name}/ssh/inventory`
   已由 KM-26（PR #8）注册、KM-28（PR #10）接通 UI；`GET/POST /api/v1/ssh/include`
   同批注册并在 SSH Inventory 页可用。这三条路径不再属于本清单（也不再禁用）。
