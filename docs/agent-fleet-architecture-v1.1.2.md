...
> **v1.1.2 增量说明（KM-18，2026-09-21）**：本版是**文档修订版**，不是一次新的架构评审或重写。它只做三件事：(1) **闭合 KM-17 独立复核报告的全部发现**（P-1～P-15，逐项处置与验收场景见 `architecture-revision-resolution-v1.1.2-supplement.md`）；(2) 落地用户 2026-09-20 决定的前端技术栈 **Svelte + TypeScript + Vite + shadcn-svelte**（改写 FR-14.4 与 §3.5，并按用户 2026-09-21 的授权登记为**用户已授权的显式 FR 例外**，见 §1.7 与 §14.5）；(3) 落地用户 2026-09-21 的另外两项裁决——两项取舍改写为「**用户已确认的取舍**」并各自写明边界（不再写作"已接受风险"，见 §2.2），八家族范围**分两批**（批次归属见 §2.2、§3.6、§5.4、§6.1、R1）。**FR 编号与统计口径一律不变**（95 条，P0 94 / P1 1）。本版为文档修订版（文档修订号 v1.1.2），**不是软件版本**。
...
| 项目 | 内容 |
|---|---|
| 文档版本 | **v1.1.2（文档修订版，非软件版本）** |
| 日期 | 2026-09-21 |
| 需求基准 | 《agent-fleet MVP Specification》v0.1（2026-09-10），本文件唯一需求来源 |
| 前序版本 | v1.1（2026-09-19，修订稿）；v1.0（2026-09-14，评审稿）——**v1.0 原文完整保留，v1.1 为在其上的完整修订，不是补丁或建议清单；v1.1.1 只在 v1.1 上落实 CA 备份决策；v1.1.2 只在 v1.1.1 上闭合 KM-17 独立复核发现并落地用户 2026-09-20 / 2026-09-21 的三个已决事项（前端技术栈、两项取舍、八家族分批），不改 FR 编号与统计** |
| 修订依据 | v1.1 依据 KM-3 第一性原理评审（M1–M5、S1–S6）、KM-12 对抗式评审报告（A1–A11、F6、小项 a–d）、KM-11 最终汇总的采纳边界（§4、§6）；v1.1.1 依据 KM-11 评论 `01a0bceb-5547-7531-b625-9957ad171b81` 的用户确认（KM-14）；v1.1.2 依据 KM-17 独立复核报告（P-1～P-15）、用户 2026-09-20 的前端栈决定（KM-15 评论 `01a0bec5-3837-75bc-b322-1a6140fd8a83`）与用户 2026-09-21 评论 `01a0c3e9-bec5-716d-93e4-46c8ddf897c3`（KM-19） |
| 修订执行 | 起草：资深架构专家（KM-13 第一次执行，未完成）；接续修订与定稿：资深后端工程师（KM-13 第二次执行）；v1.1.1 CA 备份决策同步：文档专员（KM-14）；v1.1.2 复核闭合与用户裁决落地：文档专员（KM-18） |
| 文档范围 | 需求拆解与需求分析、系统架构设计；v1.1.1 增量范围仅限 CA 备份/恢复决策及其关联表述；v1.1.2 增量范围仅限 KM-17 复核发现的逐项处置、前端技术栈改写与用户已决的三个决定，**不新增产品功能** |
| 目标读者 | 项目所有者、实现工程师（含 AI 编码代理）、测试工程师、负责执行 CA 备份与恢复的操作者 |
| 关联验收 | spec 第 34 节 12 项验收标准（A–L）、第 39 节 Definition of Done |
| 修订状态 | **待用户最终验收**：本版是修订交付物，不构成评审通过，也不构成任何软件测试结论；KM-17 复核发现已逐项闭环（处置表见 `architecture-revision-resolution-v1.1.2-supplement.md`），但**独立复核结论的验收与最终验收均由用户确认**；CA 备份/恢复**演练尚未执行**，相关验证项均为待验证 |
...
| 未决操作（Unresolved Operation） | phase ∈ {Pending, Running, **AwaitingConfirmation**, CancelRequested, Unknown} 的操作；占用该机器的服务端互斥。**合法例外**只有对**同一操作行**做状态迁移的三个端点：确认（`POST /machines/{id}/reconcile` 携 `confirmPlanDigest`）、取消（`.../operations/{opId}/cancel`）与跳过（`.../operations/{opId}/skip`）——它们不创建第二条未决操作（v1.1 新增；v1.1.2 补入 `AwaitingConfirmation`，见 §4.3、P-1） |
...
| AwaitingConfirmation | SSH 人发起路径中，plan 已产出、等待操作者确认的执行中间态；确认对象是**具体 plan**（v1.1 新增，见 §9.3）。该状态下节点**不持有**执行权（已释放），但**仍占用**控制面互斥（v1.1.2 明确，见 §4.3、§6.4） |
...
- **【v1.1.2 新增，P-13】引用约定**：本文中的 `§N.M` 指**本文自身**的小节；对需求基准 spec v0.1 的引用一律写作 `spec §N.M`（FR / NFR 表的"spec 出处"列整列指向 spec v0.1，该列内不再重复前缀）。v1.1.2 已把正文中与本文章节号可能冲突的裸 spec 引用补上前缀（§8.6、§8.7、§11.1、§11.5、§12.2、§16.1、§16.2、§16.3、§17.2、§17.3），处置记录见 `architecture-revision-resolution-v1.1.2-supplement.md`。
...
| §3.1 | 【契约】不变量表扩为 I-1～I-11，新增执行权（I-8）、事实终态（I-9）、恢复保留（I-10）、可比摘要（I-11）四条（v1.1.2 更正命名，P-12；原文误写为"代绑定、路径边界"） |
...
本小节是本版相对 v1.1 的 **CA 备份决策**的全部实质变更清单，其余章节仅为使下列位置自洽而作的措辞同步。

> **【v1.1.2 更正，P-10】记账范围更正**：除下表 12 处之外，本文件头部的「2026-09-20 Agent 范围补充」还改动了 **FR-4.2、§2.2（Agent 生态行）、§5.4（2026-09-20 补充契约）**，并在 §3.6 / §6.1 / R1 留下三家族残留（P-9）。该补充有用户截图授权，**不构成范围违规**；但 v1.1.1 原文以"全部实质变更清单"表述、且附录 A 声明"§2.4 全部 FR 逐字一致"，与文件实际状态不符。v1.1.2 在此更正记账：Agent 范围补充的改动随 v1.1.2 的批次同步一并列入 §1.7，附录 A 的原声明已在附录 A 内更正。
...
### 1.7 v1.1.1 → v1.1.2 修订摘要【复核闭环 + 用户已决】

本小节是本版相对 v1.1.1 的**全部**实质变更清单。变更来源分三类：(A) KM-17 独立复核报告的发现 P-1～P-15；(B) 用户 2026-09-20 的前端技术栈决定（KM-15）；(C) 用户 2026-09-21 的三项裁决（KM-19 评论 `01a0c3e9-bec5-716d-93e4-46c8ddf897c3`）。

| # | 位置 | 类型 | v1.1.1 原表述（摘要） | v1.1.2 修正 | 来源 |
|---|---|---|---|---|---|
| 1 | 文首增量说明 + 文档头表 | 新增/修改 | 文档版本 v1.1.1 | v1.1.2；依据加 KM-17 报告与 KM-19 评论；修订状态改为"待用户最终验收" | B、C |
| 2 | §1.3 术语表 | 修改 | 未决操作 = {Pending, Running, CancelRequested, Unknown}；AwaitingConfirmation 条目未提互斥 | 未决操作**含 `AwaitingConfirmation`**；明确三个合法例外端点；AwaitingConfirmation 条目注明"不持执行权但占用互斥" | P-1 |
| 3 | §1.5 修订摘要 | 修改 | "新增执行权、代绑定、恢复保留、路径边界四条" | 更正为"执行权（I-8）、事实终态（I-9）、恢复保留（I-10）、可比摘要（I-11）" | P-12 |
| 4 | §1.6 范围声明 | 修改 | 自称"v1.1→v1.1.1 全部实质变更清单" | 限定为 CA 备份决策的清单，并补记 Agent 范围补充实际改动的 FR-4.2 / §2.2 / §5.4 | P-10 |
| 5 | §2.2 Agent 生态行 | 修改 | 八家族并列 | 标注**批次一 / 批次二**归属 | C |
| 6 | §2.2 风险段 | 改写 | 两项未获授权的取舍写作"已接受风险" | 新增"**用户已确认的取舍**"小节（含 (a)(b) 边界）；原"已接受风险"标题改为"明确不防御项与残余风险" | P-7、C |
| 7 | §2.4 FR-1.10 | 修改 | 未决操作不含 AwaitingConfirmation | 谓词补入该 phase；声明三个合法例外端点 | P-1 |
| 8 | §2.4 FR-10.1 / FR-10.4 / FR-10.6 | 修改 | 无回滚标记；代一致性检查对回滚无分支 | 增加 `spec.rollbackOf`；回滚型 Deployment 为每机物化新代、不判 `Superseded`；`targetGeneration` 在回滚语义下为"内容代" | P-2 |
| 9 | §2.4 FR-11.6 | 修改 | "机器被删除后其证书自然到期失效" | **删除即拒绝其证书**（最短路径，不引入 CRL/OCSP） | P-7、C |
| 10 | §2.4 FR-13.9 | 修改 | "可变阶段（5–10）" | 改为"**可中断阶段（5–10）**"，与 §5.3 的"可变步骤 5–12"分离定义 | P-11 |
| 11 | §2.4 FR-14.4 | 改写 | React + TypeScript + Vite | **Svelte + TypeScript + Vite + shadcn-svelte**；登记为显式 FR 例外 | B、C |
| 12 | §2.4 FR-15.2 | 修改 | Reconcile 错误 14 类 | **15 类**（补 `RollbackUnsupported`）；显式归类 `SupersededByNewerGeneration` 与 `BlockedByNodeLock` | P-4 |
| 13 | §2.8 统计表 | 修改 | FR-11 新增列 `+2`（列合计 24 ≠ 所印 +25）；末条残留"96 项" | FR-11 改 `+3`；"96 项"改 95；补统计断言与校验脚本 | P-3、P-8 |
| 14 | §3.1 | 修改 | 串行化括注"不变量 I-8/I-9" | 改为 I-8（I-9 是事实终态不变量） | P-12 同类 |
| 15 | §3.4 流 2 / 流 3 | 修改 | 确认窗口未标注互斥状态；回滚 Deployment 未标分支 | 标注 `AwaitingConfirmation` 仍占用互斥；回滚 Deployment 走新代物化分支 | P-1、P-2 |
| 16 | §3.5 技术栈表"前端"行 | 改写 | React + TypeScript + Vite SPA（推荐项） | Svelte + TypeScript + Vite + shadcn-svelte | B |
| 17 | §3.6 代码布局与依赖约束 | 修改 | `codex/ omp/ opencode/` | `adapter/<family>/` + 批次一示例；依赖约束改为家族实现包的一般化表述 | P-9 |
| 18 | §4.2 职责 1 与条件写入规则表 | 改写 | "维护 conditions（唯一写入方）"；规则表无置位方列 | 改为"每个 condition 恰有一个置位方"（以 §6.2 归属为准）；规则表**补置位方列**；末行"操作终态"改"操作处于…时" | P-5、P-14 |
| 19 | §4.3 / §4.9 唯一索引谓词 | 修改 | `phase IN ('Pending','Running','CancelRequested','Unknown')` | 补 `'AwaitingConfirmation'`；§4.3 新增第 6 条"互斥的合法例外" | P-1 |
| 20 | §4.4 回滚与代一致性 | 修改 | 回滚仅"创建新 Deployment 指向历史 targetGeneration" | 增加回滚分支与"内容代"语义、失败码 `RollbackUnsupported` | P-2 |
| 21 | §4.6 机器删除后的证书 | 改写 | 删除后证书到期前仍可建立 Connect 流 | **删除即拒绝**其证书的后续 Connect；UI 标注"当前限制：无证书吊销" | P-7、C |
| 22 | §4.7 工件预算表 | 修改 | 三项预算 | **四项**（补"单 Skill 工件（归档/上传态）大小"，默认值待负载验证） | P-6 |
| 23 | §5.2 | 修改 | "自动 apply 的第一阶段之内的写操作" | 改为"`ExecuteOperation`，含自动 apply 派发的操作" | P-14 |
| 24 | §5.3 | 新增/修改 | 只有"可变步骤 5–12" | 新增"可中断阶段 5–10 / 不可中断阶段 11–13"术语与用途分离声明 | P-11 |
| 25 | §5.4 | 修改 | 新增家族未标批次 | 标注批次一 / 批次二 | C |
| 26 | §6.1 | 修改 | `agents（codex/omp/opencode：…）`；Deployment 无回滚字段 | 家族键 + 批次归属；Deployment 增 `rollbackOf` | P-9、P-2 |
| 27 | §6.4 | 修改 | Reconcile 14 类；确认窗口与互斥的关系未闭环 | 15 类；显式归类两个非错误码枚举；补确认窗口互斥规则 | P-1、P-4 |
| 28 | §7.2 | 修改 | 回滚产生新代（未说明每机独立计数与 Deployment 表达） | 明确**每机各自新代**与 `targetGeneration` = 内容代；回滚 API 补 `rollbackOf` | P-2 |
| 29 | §7.3 / §7.4 | 修改 | 跳过与预算未写边界 | 跳过补审计与"不计入成功"边界；预算补工件大小项 | P-7、P-6 |
| 30 | §8.1 | 修改 | 确认端点未标为互斥例外 | 标注三个例外端点 | P-1 |
| 31 | §9.3 | 新增 | 无 | **确认窗口内的并发处置规则 5 条**（新操作被拒、例外端点、重算基线、超时处置、取消/跳过差异） | P-1 |
| 32 | §9.6 路径 C | 修改 | 前提"无，风险由操作者承担" | 边界：审计记录 + UI 显式标出 + 不允许静默跳过 + 不计入成功 | P-7、C |
| 33 | §9.7 | 修改 | 阶段 5-10 / 11-13 表述 | 统一为"可中断阶段 / 不可中断阶段"并回引 §5.3 | P-11 |
| 34 | §11 威胁模型第 2 点 | 改写 | "删除机器不是安全措施"（无最小路径） | 删除即拒绝 + 标注"当前限制：无证书吊销" + 安全需求走分支三 | P-7、C |
| 35 | §13.1 T2 / T10 / T16 | 修改 | 未覆盖确认窗口、四项预算、回滚可达性 | 三处扩展断言（P-1 / P-6 / P-2 的验收场景） | P-1、P-2、P-6 |
| 36 | §13.3 / §13.4 | 修改 | J 未列 FR-10.x；反例表 19 行 | J 补 FR-10.1/10.4/10.6；追加 2 条反例（共 21 行） | P-2 |
| 37 | §14.4 | 修改 | 两项写"已接受风险" | 改为"用户已确认的取舍"并指向 §2.2 边界 | P-7、C |
| 38 | §14.5（新增） | 新增 | — | **用户已授权的显式 FR 例外：FR-14.4 前端技术栈**（来源、范围限定、检查记录） | B、C |
| 39 | §16 第 11 项 | 修改 | "MVP 明确不做（§2.2 已接受风险）" | 改为"只实现删除即拒绝的最短路径" | P-7 |
| 40 | §17 R1 / R7 | 修改 | R1 写三家族；R7 写已接受风险 | R1 八家族分批 + 批次二身份待实测；R7 用户已确认取舍 + 删除即拒绝 + UI 标注 | P-9、P-7、C |
| 41 | §18 | 修改 | 统计口径段；本版状态 | 补统计断言与校验脚本；补 v1.1.2 边界修正段与"待用户最终验收" | P-3、C |
| 42 | 附录 A / 附录 B | 修改/新增 | 附录 A"§2.4 全部 FR 逐字一致"失实 | 更正该声明；新增**附录 B：v1.1.2 触及位置索引** | P-10 |
| 43 | spec §24（关联文件，非本文件） | 修改 | "React + TypeScript + Vite is recommended for MVP" | 改为 Svelte + TypeScript + Vite + shadcn-svelte 并附修订记录（2026-09-20 用户决定） | B |
| 44 | §1.4 阅读指引 + 正文 10 处引用 | 新增/修改 | 无引用约定；`§8.6`/`§8.7`/`§11.1`/`§11.5`/`§12.2`/`§16.1`/`§16.2`/`§16.3`/`§17.2`/`§17.3` 等裸引用与本文自身章节号冲突 | 新增"引用约定"（本文用 `§N.M`、spec 用 `spec §N.M`）；对上述 10 个冲突引用加 `spec ` 前缀 | P-13 |
| 45 | `architecture-revision-resolution.md`（关联文件） | 修改 | §6.1"96 条 0 命中"、§6.2"错误码无未分组码""§4.2 全部标注置位方""§13.4 18 类反例"四处失实声明 | 追加**更正块**（保留原文并标注 v1.1.2 更正口径：96 项口径、`RollbackUnsupported` 未归类、置位方列缺失、19 行），指向本版 supplement | P-4、P-5、P-8、P-15 |

**范围边界（v1.1.2）**：除 FR-14.4 这一条
...
| Agent 生态 | Claude、Codex、DeepSeek Harness、Grok、Hermes、Oh-My-Pi（OMP）、ZCode、OpenCode 八个 MVP 支持目标，**分两批接入**（用户 2026-09-21 决定，KM-19）：**批次一（MVP 先打通）**＝Codex、OMP、OpenCode、ZCode；**批次二（批次一打通、确认适配器抽象站得住后再接入）**＝Claude、DeepSeek Harness、Grok、Hermes。批次归属只表示范围与接入顺序，**不改变任何家族的验收要求**，也不改变 FR 计数与统计口径（§2.8）。身份与能力验证见 [Agent 支持矩阵](agent-support-matrix.md)；Gemini CLI 仍为后续扩展 |
...
**用户已确认的取舍（2026-09-21，来源：KM-19 评论 `01a0c3e9-bec5-716d-93e4-46c8ddf897c3`）**：

以下两项曾在本文件 v1.1 / v1.1.1 中被写作"已接受风险"，但 spec 未授权、用户当时也未确认；用户已于 2026-09-21 明确**接受**，并接受下列边界。本版因此把它们记为**用户已确认的取舍**（不再是"待确认"，也不再表述为"已接受风险"），边界为强制约束：

- **(a) MVP 不做证书吊销（无 CRL/OCSP）**。取舍内容：MVP 不实现吊销列表与在线证书状态查询。**边界（必须实现的最短路径）**：**删除机器即拒绝其证书**——控制面在机器被删除后**拒绝该机器已签发证书的后续连接**，并记录被拒事实；**不引入**完整 PKI / HSM / CRL / OCSP。UI 与运维口径必须标注「**当前限制：无证书吊销**」及删机语义（§4.6、§11、R7）：删除机器**不是**让证书立即失效的通用安全措施，真正的安全需求走 §4.6 分支三（重建信任）。
- **(b) Unknown 目标允许人工跳过**。取舍内容：操作状态无法判定时，允许操作者手工跳过该目标。**边界**：跳过必须留**审计记录（操作者、时间、目标、原因）**并在 **UI 显式标出**；**不允许静默跳过**；跳过**不改变目标的实际状态**，也**不得计入成功**（终态为 `Superseded(Skipped)`，并保留"节点可能仍在写"的提示，见 §9.6 路径 C、§7.3）。
...
| **FR-1.10【新增】** | **未决操作可见与阻塞**：机器存在未决操作（**Pending/Running/AwaitingConfirmation**/CancelRequested/Unknown）时，UI 必须显示原因，且新的变更类操作默认被拒绝（返回 `MachineBusy`），除非操作者显式取消/跳过；此约束对 daemon 与 SSH 两条通道一致生效。**【v1.1.2 补入 AwaitingConfirmation，P-1】合法例外仅限对同一操作行的确认/取消/跳过端点**（§4.3 第 6 条、§9.3） | P0 | §35.2 |
...
| 编号 | 需求 | 优先级 | spec 出处 |
|---|---|---|---|
| FR-10.1 | Deployment 资源：显式机器列表、`targetGeneration`、**回滚标记 `spec.rollbackOf`（指向被回滚的 generation，非空即回滚型 Deployment）**、策略（canary 数、整数 batchSize、maxUnavailable、pauseOnFailure）。**【v1.1.2 新增，P-2】回滚型 Deployment 的 `targetGeneration` 语义为"内容代"**（要重放的内容来自哪一代），逐机的新代编号由控制器在推进时物化并记录 | P0 | §8.6、§21.1 |
...
| FR-10.4 | 回滚 = 创建指向既往 recorded generation 的新 Deployment（`spec.rollbackOf`，见 §7.2）；历史不可变。**【v1.1.2 明确，P-2】回滚 Deployment 不把机器拉回旧代**：它为每台目标机物化**新代**（内容 = 目标代内容），因此不适用"当前代高于目标代即 `Superseded`" | P0 | §21.4 |
| FR-10.5 | Deployment 暂停/恢复/回滚 API 与 UI 进度（含逐机结果） | P0 | §23.5、§24.6 |
| **FR-10.6【新增】** | **目标代一致性检查**：Deployment 推进某目标前必须校验 `targetGeneration` 仍等于该机当前 generation；若机器当前代已高于目标代，该目标置 `Superseded` 并**不**执行（防止把机器"拉回旧代"造成回退震荡；回退只能由显式 rollback 表达）；若低于目标代，等待其收敛。**【v1.1.2 分支，P-2】回滚型 Deployment（`spec.rollbackOf` 非空）不适用上述判定**：推进前先把目标代**内容**解析为该机的新期望代（按 §7.2 物化新代，内容 = 目标代内容），再以该新代作为**有效目标代**参与门禁与后续校验；因此回滚路径不会被自身的代一致性检查判 `Superseded`，验收 J 可达。目标代内容不可用 → 该目标 `Failed(RollbackUnsupported)` | P0 | §21.2、§21.4 |
...
| **FR-11.6【新增】** | **重复连接与删除机器策略**：同一 `machineId` 的并发第二条 `Connect` 流默认**拒绝并产生告警事件**（不做"新连接挤掉旧流"）；**【v1.1.2 修订，用户已确认取舍 (a)】删除机器即拒绝其证书**：机器被删除后，控制面**必须拒绝该机器已签发证书的后续 `Connect`**（被拒事实连同 machineId、证书序列号与时间写入审计），而不是仅等待其自然到期；MVP **不引入** CRL/OCSP（§2.2、§4.6）。`agent_certificates` 保留序列号与删除时间作为审计事实 | P0 | §11.1、§22 |
...
| **FR-13.9【新增】** | **取消语义**：`CancelOperation` 仅在阶段边界生效；处于**可中断阶段（5–10）**时取消 = 走标准备份恢复路径后终态 `Failed(Cancelled)`；重复取消幂等；节点不可达时取消**不生效**，服务端不得单方面把操作置为 `Cancelled`——在收到节点结果前只能保持 `CancelRequested`。**【v1.1.2 术语统一，P-11】「可中断阶段 5–10 / 不可中断阶段 11–13」用于取消，「可变步骤 5–12」用于失败恢复**（§5.3、§9.7） | P0 | §11.4、§14.2 |
...
| FR-14.4 | SPA 技术栈（**用户 2026-09-20 定案**）：**Svelte + TypeScript + Vite**，页面组件使用 **shadcn-svelte**（按页面需要引入）；API 契约不得依赖前端框架。**【v1.1.2 改写，用户已授权的显式 FR 例外】**本次改写与 §1.6"不新增、不改写任何 FR"的边界冲突，已按用户 2026-09-21 授权登记为例外（§1.7、§14.5） | P0 | §24 |
...
| FR-15.2 | 错误模型四组：SSH 错误（6 类）、Agent 错误（6 类）、Reconcile 错误（**15 类**，含 v1.1.2 补入的 `RollbackUnsupported`）、操作协调错误（4 类）；每条用户可见错误含 reason code、人读消息、operation ID、时间戳、安全诊断。**【v1.1.2 修订，P-4】另有非错误码的展示与终态枚举**：Deployment 终态 reason `SupersededByNewerGeneration`、phase 展示修饰 `BlockedByNodeLock`，二者在 §6.4 显式归类 | P0 | §30 |
...
| v1.1 新增 | +2 | 0 | 0 | 0 | 0 | +2 | +1 | +2 | +4 | +2 | **+3** | +2 | +4 | +1 | +2 | **+25** |
...
- **FR-2.6 的性质【澄清】**:它是"未来扩展约束"（spec §37 明确把原生 installer drivers 放在 Post-MVP），不表示 MVP 必须实现 95 项功能。（v1.1.2 更正：v1.1.1 此处残留"96 项"，P-8）

**统计校核记录**：本节数字与 §18 结论段数字必须一致，且二者都由机械统计产生。**【v1.1.2 新增断言，P-3】校验脚本 `docs/tools/verify-architecture-stats.py` 必须通过以下断言**：(1) 每行 15 列合计 == 该行所印合计（v1.0 行 = 70、v1.1 新增行 = +25、v1.1 行 = 95）；(2) 逐列满足"v1.0 条数 + v1.1 新增 = v1.1 条数"；(3) §2.4 表格机械统计得出 95 条 / P0 94 / P1 1；(4) §2.8、§18 与附录 A / 附录 B 的功能需求条数均为 95，且全篇不存在作为本版数字的 96/70/+26。断言未通过即视为文档缺陷，不得只改文字。历史验证记录见 `architecture-revision-resolution.md` §6.1；其"96 条 0 命中"的残留检查口径已在 v1.1.2 更正为同时匹配"96 条 / 96 项"。
...
因此 v1.1 明确：**串行化必须同时在节点侧与持久层表达**（不变量 I-8，机制见 §5.6 与 §4.3）。（v1.1.2 更正引用：I-9 是"操作终态必须由事实产生"，不是串行化不变量。）
...
→ 节点取得执行权（§5.6）→ 产出 plan 与基线（观测受管摘要 + inventorySeq + 快照摘要）→ 服务端记录 planDigest → Operation 进入 AwaitingConfirmation（**该 phase 仍占用控制面互斥**，故确认窗口内新的变更操作被 409 `MachineBusy` 拒绝；例外见 §4.3 第 6 条、§9.3）
...
→ 目标代一致性检查：targetGeneration < 机器当前 generation → 该目标置 Superseded（FR-10.6）；**若为回滚 Deployment（`spec.rollbackOf` 非空）则改走回滚分支——按 §7.2 为该机物化新代（内容 = 目标代内容）后推进，不判 Superseded**
...
→ 操作者修复后 resume 或创建回滚 Deployment（`spec.rollbackOf` 指向旧 generation；语义见 §7.2）
```
...
| 前端 | **Svelte + TypeScript + Vite SPA**，页面组件使用 **shadcn-svelte**（按页面需要引入；用户 2026-09-20 定案） | §24；API 契约不绑定框架（NFR-9 不变） |
...
agent-fleet/
  ...
  internal/
    ...
    agentlocal/               # —— 仅 agentd 二进制链接本组包 ——
      ...
      adapter/                # 适配器注册表与公共契约
        <family>/             # 家族实现（路径/格式/合并策略封装于此）；家族集合与批次归属见 agent-support-matrix.md 与 §2.2（批次一：codex/omp/opencode/zcode）
  web/                        # SPA（Svelte + TypeScript + Vite + shadcn-svelte 组件，§3.5）
...
- `controller/*`、`server/*` 不得 import 任何 `agentlocal/adapter/<family>` 家族实现包（护栏 #2、§27；v1.1.2 更正：原文以 `{codex,omp,opencode}` 举例，易被读成"只有三家族"）；
...
1. 维护 conditions 的**写入纪律**：**每个 condition 恰有一个置位方**（Machine 控制器写 `SSHReachable` / `AgentConnected` / `InventoryReady`；Reconcile 控制器写 `Drifted` / `Reconciled` / `Degraded`，见 §6.2）。Machine 控制器**不是**全部 conditions 的写入方，也不得写其它控制器的 condition。每次写完整记录（status/reason/message/lastTransitionTime），条件间不互相推导（§22）。（v1.1.2 修订，P-5：原文"维护 conditions（唯一写入方）"与 §6.2 的归属冲突）
...
| 事件 | 写入条件 | 置位方（v1.1.2 补，P-5） |
|---|---|---|
| SSH 探测成功/失败 | `SSHReachable=True/False`（失败带 §30.1 的 reason）；同时写 `lastProbeAt` | Machine 控制器 |
| Connect 流建立/断开 + 45s 无心跳 | `AgentConnected=True/False` | Machine 控制器（由 gRPC 连接注册表驱动） |
| 收到首个有效 inventory | `InventoryReady=True`（同时写 `lastInventoryAt`、`inventorySeq`、`observedGeneration`） | Machine 控制器 |
| 观测受管投影摘要 ≠ 期望受管投影摘要（同一 canonicalizationVersion）/ 相等 | `Drifted=True/False`、`Reconciled=False/True` | Reconcile 控制器 |
| **【新增】从未采集过 inventory** | **`Drifted=Unknown`、`Reconciled=Unknown`（reason=`NeverInventoried`）——不得写 False** | Reconcile 控制器 |
| **【新增】观测新鲜度超过配置阈值** | **`Drifted=Unknown`（reason=`StaleObservation`），保留 `lastInventoryAt` 供 UI 展示"上次确认于 T"** | Reconcile 控制器 |
| **【新增】期望代已变但尚未据新代采集** | **`Drifted=Unknown`（reason=`ObservationPredatesDesired`），直到该代操作完成并上报新观测** | Reconcile 控制器 |
| 恢复失败 | `Degraded=True`（后续 rollout 跳过该机，§14.2） | Reconcile 控制器 |
| **【新增】操作处于 CancelRequested/Unknown 时** | **不写 `Drifted`/`Reconciled`；该机由"未决操作"状态单独呈现** | Reconcile 控制器（不写，保持既有值） |

> **【v1.1.2 说明，P-5】**：上表与本表新增的"置位方"列**以 §6.2 的归属为唯一权威**；§6.2 与本节不一致时以 §6.2 为准。原 resolution 中"§4.2 条件表全部标注置位方"的声明此前与文件实际不符，v1.1.2 已补列并在 `architecture-revision-resolution.md` 附更正。
...
1. 互斥的载体是 `operations` 表上的**部分唯一索引**：
   ...
   ```sql
   -- 每台机器至多一个"未决"操作（含 Unknown 与等待确认，P-1）
...
2. **派发点获取**：Reconcile 控制器在派发前以一次事务创建 `phase=Pending` 的操作行；唯一约束冲突即表示该机已有未决操作 → 请求返回 409 `MachineBusy`（FR-1.10），不排队（MVP 用"拒绝 + 显式取消/跳过"替代复杂队列，宁停不错）。**【v1.1.2 新增，P-1】确认窗口内的并发处置规则**：进入 `AwaitingConfirmation` 的操作**仍是未决操作**，其互斥直到确认后 apply 终结、取消获节点确认、显式跳过或计划超时释放（§9.3）；
...
6. **【v1.1.2 新增，P-1】互斥的合法例外**：`POST /machines/{id}/reconcile` 携 `confirmPlanDigest`（确认**该操作自己的**计划）、`POST /machines/{id}/operations/{opId}/cancel`、`POST /machines/{id}/operations/{opId}/skip` 是唯一三个例外——它们**不创建新的未决操作**，只对同一 `operationId` 的操作行做状态迁移，因此不违反唯一索引。除这三个端点外，任何变更类路径（含 Deployment 派发、自动 apply、SSH oneshot apply）都必须走第 2 条的派发点获取。

**【契约】并发上限与互斥的关系**：全局 ≤20 并发变更操作（NFR-1）是**资源上限**，机器级互斥是**正确性约束**。两者独立：20 是"能同时推进多少台不同机器"，机器级互斥是"同一台机器不能有两个变更"。

### 4.4 Deployment 控制器

- 状态机：`Pending → Canary → RollingOut → (Paused | Succeeded | Failed)`（phase 命名遵循 spec §8.6 的 `RollingOut` 风格）；
...
- 回滚：`POST /deployments/{id}/rollback` 创建新 Deployment（`spec.rollbackOf` = 被回滚的 generation）指向历史 `targetGeneration`；历史记录不可变（§21.4）。**【v1.1.2 明确，P-2】回滚型 Deployment 的目标代是"内容代"**：每台目标机按 §7.2 物化自己的**新代**（内容 = 目标代内容），该新代才是门禁与后续校验使用的**有效目标代**，因此回滚不会被下面的代一致性检查自我阻塞；
...
- **【v1.1.2 新增，P-2】回滚型 Deployment（`spec.rollbackOf` 非空）** → **不适用前三条**：控制器先把 `targetGeneration` 的**内容**解析为该机的新期望代（§7.2：重放旧内容、产生新代），以该新代作为有效目标代再评估门禁；目标代内容不可用（快照/工件已被清理或损坏）→ 该目标 `Failed(RollbackUnsupported)` 并提示操作者（§10.4）。
...
- 首启本地生成 Fleet CA（自签），`pki/ca.key` 权限 0600（spec §12.2、spec §26）；
...
- `RenewCertificate`：仅接受既有有效 mTLS 通道上的续期请求（spec §12.2），**续期窗口加随机抖动**（避免全机群续期对齐到同一时刻，S4）；
- 服务端证书：`pki/server.crt/key`，advertiseURL 的 TLS 终结点使用；agentd 引导配置需携带 CA 证书以校验服务端；
- **【v1.1.2 修订，用户已确认取舍 (a)】机器删除后的证书**：MVP 无吊销机制（无 CRL/OCSP，见 §2.2 用户已确认的取舍）。**删除机器即拒绝其证书**：机器被删除后，控制面**必须拒绝该机器已签发证书的后续 `Connect`**（按库中删除事实判定，拒绝并记录 machineId、证书序列号与时间），不得仅以"证书未到期"放行；`agent_certificates` 保留序列号与删除时间作为审计事实。**边界**：本路径不引入完整 PKI / HSM / CRL / OCSP，也不改写证书有效期语义；UI 与运维口径必须标注「**当前限制：无证书吊销**」——"删除即拒绝"是**最短路径**，**不等价于**通用吊销能力。**删除机器仍然不是一般的安全措施**（删除前证书材料可能已被复制或借用），真正的安全需求走本节第 7 条分支三（重建信任）；
- **【v1.1.1 重写】CA 材料的人工备份、恢复与演练契约【用户已决，KM-14】**：用户已确认（KM-11 评论 `01a0bceb-5547-7531-b625-9957ad171b81`）**MVP 不新增内置 CA 备份导出产品功能**，同时**必须提供人工安全备份、恢复与演练方案**。因此 v1.1 的"不做离线备份＝已接受风险"表述作废，替换为下列契约。完整可执行步骤、前置条件与停止点见 `manual-ca-backup-recovery.md`；本节只固定边界、责任与验收口径。
  ...
  3. **备份内容及边界**（缺一不可，但粒度按恢复目标选择）：
     ...
     - **不在备份范围**：任何 API key 值、SSH 私钥内容（产品本就不持久化，spec §11.1/11.2）；节点侧 `client.key`（属于节点本地，丢失时按本节第 7 条分支二重新 enroll）。
...
| 项 | 默认值 | 超限行为 |
|---|---|---|
| **单 Skill 工件（归档/上传态）大小【v1.1.2 补，P-6】** | **32 MiB（本版设计默认值，待负载验证）** | 解析期拒绝，`ArtifactTooLarge`，**不发生网络传输** |
...
**【v1.1.2 说明，P-6】**：FR-6.8 要求**四项**预算（单 Skill 工件大小、解包后总大小、文件数、单 bundle 总量），v1.1.1 的表只落三项——缺"单 Skill 工件（归档/上传态）大小"这一维，而 S1 的原始问题（"传了十分钟才失败"）针对的正是**上传前**的大小。本版补齐为四项，并把该项默认值显式标注为**待负载验证的设计默认值**，不得读作实测结论。
...
- 连接：出站 mTLS gRPC `Connect` 双向流；指数退避 + 抖动重连；每次（重）连接先 `Hello`（身份、版本、支持的 adapters/schema 版本）+ 全量 `ObservedState`（spec §11.5、spec §31）；
...
- **【契约】变更执行一律入单 worker 队列**：daemon 内所有变更（`ExecuteOperation`，**含自动 apply 派发的操作**——按 FR-9.8，自动 apply 是控制面下发的普通 Operation，不存在"节点内第一阶段"）进入**同一个**串行队列，同一时刻只有一条流水线运行（FR-9.9 的进程内一半）。（v1.1.2 措辞更正，P-14）
...
- 工件获取：`FetchArtifact(digest)` 认证流式下载 → 本地 SHA-256 校验 → 路径边界校验 → 进入规范缓存（spec §17.2、FR-6.7）；
- 证书续期：到期前经既有通道调 `RenewCertificate`（spec §12.2），窗口带随机抖动；
...
**【v1.1.2 术语统一，P-11】三组边界必须分开引用**（v1.1.1 的 FR-13.9"可变阶段 5–10"与本节"可变步骤 5–12"曾同名不同义）：

| 术语 | 阶段范围 | 用途 | 出处 |
|---|---|---|---|
| **可变步骤** | **5–12** | **失败恢复**：任一可变步骤失败即触发"停止后续 → 恢复 → 再 inventory → 上报" | 本节、§14.2 |
| **可中断阶段** | **5–10** | **取消**：取消在此范围内生效，走标准备份恢复路径 | §9.7、FR-13.9 |
| **不可中断阶段** | **11–13** | **取消**：为避免在验证/提交阶段制造半状态，取消在此范围不生效，操作继续到终结 | §9.7、FR-13.9 |

失败行为（§14.2）：**可变步骤 = 阶段 5–12**（一切可能改变节点状态、或据其判定操作成功的步骤）。
...
新增目标家族（Claude、DeepSeek Harness、Grok、Hermes、ZCode）及后续家族（如 Gemini CLI）接入 = 新增 `adapter/<family>` 包并注册：实现上述接口 + 提供 fixture home 测试，不改控制面核心（§1 的扩展性要求由此满足）。**【v1.1.2】接入顺序按用户 2026-09-21 决定分两批**：批次一（MVP 范围）= Codex、OMP、OpenCode、ZCode；批次二 = Claude、DeepSeek Harness、Grok、Hermes（批次一打通、确认适配器抽象站得住后再接入）。批次归属只表示范围与顺序，**不降低任何家族的验收要求**，也不改变 FR 计数与统计口径（§2.8）。
...
- 文本 rules：`# BEGIN/END agent-fleet managed` 标记块替换；不可安全部分拥有的文件必须声明 `ownership=whole-file` 且 UI 可见（spec §16.2）；
...
| AgentProfile | 操作者 CRUD | agents（**按家族键**：enabled/version/installer/config；家族集合、能力声明与批次归属见 [Agent 支持矩阵](agent-support-matrix.md) 与 §2.2，批次一 = codex/omp/opencode/zcode）、skills[]（skillRef）、mcp{}、rules | —（无控制器） |
...
| Deployment | 操作者创建 + 控制器写 status | selector.machineNames、targetGeneration、**rollbackOf【v1.1.2 新增，P-2】**、strategy（canary/batchSize/maxUnavailable/pauseOnFailure） | phase、succeeded/failed/pending/**superseded/skipped【新增】** |
...
- 错误码四组（31 码：6+6+**15**+4，§30）：SSH 六类（DNSResolveFailed、HostKeyVerificationFailed、AuthenticationFailed、ConnectionTimeout、RemoteCommandFailed、UnsupportedPlatform）/ Agent **六类**（AgentDisconnected、ProtocolVersionMismatch、EnrollmentRejected、CertificateExpired、InventoryFailed，+ **v1.1 增补 `ProjectionVersionMismatch`**，见 §7.1）/ Reconcile **十五类**（v1.0 九类 DesiredStateInvalid、InstallerFailed、VersionVerificationFailed、ConfigParseFailed、ConfigWriteFailed、SkillDownloadFailed、SkillDigestMismatch、HealthCheckFailed、RollbackFailed，+ **v1.1 增补** `ReplanRequired`、`VerifyFailed`、`RestoreConflict`、`ArtifactTooLarge`、`SkillPathRejected`，+ **v1.1.2 补入 `RollbackUnsupported`，P-4**：spec §20.3 强制、正文使用于 FR-2.4 / §2.3 U6 / §7.2 / §10.4）/ 操作协调**四类**（`MachineBusy`、`NodeBusy`、`StaleExecutionLock`，+ **v1.1 增补 `BundleTooLarge`**，见 §7.4）；
- **【v1.1.2 新增，P-4】非错误码的展示与终态枚举（显式归类）**：`SupersededByNewerGeneration` 是 **Deployment 终态 reason**（与 `Skipped` 同表列出，见 §4.4、§6.1），**不属**上述四组错误码；`BlockedByNodeLock` 是 **phase 的展示修饰**（daemon 因节点执行权忙而排队时使用，见 §5.6 规则 3），**不属**错误码。因此"错误码全篇一致、无未分组码"的正确口径是：**除被显式归类为 phase / terminalModifier / Deployment 终态 reason 的枚举外，每个被引用的 reason code 都能在四组中找到归属**（FR-15.2）。
...
**【契约】pending 阶段与执行权的关系**：`AwaitingConfirmation` 表示 plan 已产出、等待确认，此时节点**不持有执行权**（已释放），控制面互斥**仍持有**。这是两条独立机制的自然分工：执行权管"文件正在被写"，互斥管"这台机器不要开新操作"。**【v1.1.2 明确，P-1】该 phase 属于 §1.3 的"未决操作"集合、命中 §4.3 与 §4.9 的唯一索引谓词**：确认窗口内任何新的变更类操作都会被 409 `MachineBusy` 拒绝，Deployment 该目标阻塞（FR-10.7），UI 呈现"等待确认（剩余 T）"；唯一例外是对同一操作的确认/取消/跳过端点（§4.3 第 6 条）。
...
3. Skill 的 drift 信号 = 目标 revision/内容 digest（symlink 模式）或复制内容 digest（copy 模式，spec §17.3）；
...
备份（spec §16.3）：每次变更前把受管工件复制到 `~/.local/share/agent-fleet/backups/<operation-id>/`，附恢复所需元数据（原文件路径、权限、内容哈希、**受管键集合与值**、Skill link 目标、先前版本号）；保留每机最近 10 次操作（可配置）。
...
回滚 API：`POST /machines/{id}/rollback`（机器级，参数为目标 generation）；`POST /deployments/{id}/rollback`（机群级，生成 `spec.rollbackOf` = 被回滚 generation 的新 Deployment，逐机物化新代，§21.4、FR-10.1/10.4/10.6）。

**【契约】回滚与 generations 的交互**：
- 回滚**产生新的 generation**（重放的是旧内容，但期望状态相对上一代发生了变化），而不是"把 machine.generation 改回旧值"——历史不可变（§21.4）。**【v1.1.2 新增，P-2】generation 是**每机独立**的计数**（§6.3 按 `(machine_id, generation)` 唯一），因此机群级回滚为**每台目标机各自**物化一个新代（A 机 gen15、B 机 gen09…）；Deployment 的 `spec.targetGeneration` 在回滚型 Deployment 上因此是"**内容代**"（要重放的内容来自哪一代），而**不是**"目标机应达到的代编号"——逐机的新代编号由控制器在推进时物化并记录（FR-10.1、FR-10.6、§4.4）；
...
- **【新增】未决目标**：目标机有未决操作（含 `AwaitingConfirmation` 与 `Unknown`）时该目标阻塞、批次不推进（FR-10.7）；操作者可在 UI 显式跳过（终态 `Superseded(Skipped)`）。**跳过的边界（用户已确认取舍 (b)）**：必须留审计记录（**操作者、时间、目标、原因**）并在 UI 显式标出，**不允许静默跳过**；跳过不宣称节点已停止、**不改变目标的实际状态**、**不计入成功**（§2.2、§9.6 路径 C）；
...
6. **【新增】预算**：超过 §4.7 的 bundle 预算时不构建、不派发，报 `BundleTooLarge`；**【v1.1.2，P-6】单 Skill 工件（归档/上传态）大小**同样在解析期校验（§4.7 四项预算）。
...
证书生命周期（spec §12.2）：有限期客户端证书；到期前经已认证通道续期（`RenewCertificate`，**窗口加随机抖动**）；CA 私钥**由产品全程留在控制面主机**（0600），产品不将其外发、不写入日志/工件/快照，也不提供导出端点。续期失败/过期 → `CertificateExpired` 错误，UI 提示重新 bootstrap（经 SSH）。
...
- **【新增】SSH 路径的确认端点**：`plan` 产出的计划以 `planDigest` 标识，操作者确认的对象是它。确认动作沿用 `POST /machines/{id}/reconcile`（携带 `confirmPlanDigest`），以便服务端在确认时校验"确认的还是当初那一份计划"。**【v1.1.2，P-1】该端点与 `.../operations/{opId}/cancel`、`.../operations/{opId}/skip` 是机器级互斥的三个合法例外**（对同一 `operationId` 做状态迁移，不创建第二条未决操作，§4.3 第 6 条、§9.3）。
...
- 服务端幂等处理重复 OperationResult（按 operationId 去重，spec §11.5）；**Ack 之后节点方可淘汰 outbox 项**；
...
**【v1.1.2 新增，P-1】确认窗口内的并发处置规则【契约】**：`AwaitingConfirmation` 期间节点**不持有**执行权、控制面**仍持有**互斥（§6.4），因此：

1. **新的变更类操作被拒绝**：该机在此期间收到的手动 reconcile、Deployment 派发或自动 apply 一律返回 409 `MachineBusy`（FR-1.10），Deployment 目标按 FR-10.7 阻塞；UI 呈现"等待确认（剩余 T）"并给出取消入口。
2. **同一操作的三个例外端点可用**：确认（`POST /machines/{id}/reconcile` 携 `confirmPlanDigest`）、取消（`.../operations/{opId}/cancel`）、跳过（`.../operations/{opId}/skip`）只对该操作行做状态迁移，不视为新操作（§4.3 第 6 条）。
3. **确认不等于"计划仍有效"**：`oneshot apply` 仍必须重取观测、重算 plan 并比对基线，基线不一致 → 拒绝执行、零变更、报 `ReplanRequired`，需重新 plan 与重新确认（FR-12.7）。
4. **超时处置**：计划超时后该操作以 `Failed` 终结，审计保留原 `planDigest` 与超时时刻，**互斥同时释放**；不得把超时计划继续推进。
5. **取消与跳过的差异**：取消需节点确认停止（节点可达时）才离开未决集合；跳过按用户已确认取舍 (b) 立即释放控制面互斥，但必须留审计记录（操作者、时间、目标、原因）并在 UI 显式标出，**不允许静默跳过**，且**不计入成功**（§2.2、§9.6 路径 C）。
...
| C. 显式跳过 | 操作者 POST skip（须填原因） | `Superseded(Skipped)`，释放控制面互斥，**但保留"节点可能仍在写"的提示与 Degraded/Unknown 标记**；**不计入成功** | 无。**边界（用户已确认取舍 (b)，2026-09-21）**：跳过必须留审计记录（操作者、时间、目标、原因）并在 UI 显式标出，**不允许静默跳过**；跳过**不改变目标的实际状态**（§2.2） |
...
→ 节点在阶段边界检查取消标志：
   ...
   · 阶段 1-4 之间（可中断阶段之前） → 直接终止，终态 Failed(Cancelled)，无需恢复（尚无变更）
   · **可中断阶段 5-10** 之间 → 停止后续 → 走标准备份恢复路径（含外部编辑冲突检测）→ 终态 Failed(Cancelled)
   · **不可中断阶段 11-13** 之间 → 视为不可中断，继续到终结（避免在验证/提交阶段制造半状态）
   · 取消边界与"可变步骤 5–12（失败恢复）"的术语对照见 §5.3（v1.1.2 统一，P-11）
→ 节点回报 → 服务端终结操作 → 释放互斥
...
| 12 | 配置原子写 | 临时文件 + fsync + rename + 写后解析验证（spec §16.1） |
...
2. **机器删除只做到"删除即拒绝"（用户已确认取舍 (a)，2026-09-21）**：MVP 无 CRL/OCSP，但控制面**必须拒绝已删除机器证书的后续 `Connect`**（§4.6、FR-11.6）。因此"删除机器"能阻止该证书**继续正常使用**，但它是"拒绝该 machineId"的最短路径，**不是**通用吊销能力：删除**之前**已被复制或借用的证书材料、以及既有信任链本身，都不因删除而失效。安全需求（如怀疑私钥泄露）的正确做法仍是显式重新生成 CA 并重新 bootstrap 全机群（§4.6 分支三），这是运维决策而非自动行为。UI 与运维口径必须标注「**当前限制：无证书吊销**」。
...
| T2 | **服务端持久化互斥** | 派发 op1 后重启控制面进程，立即经 SSH 触发 reconcile：op2 被拒（409 MachineBusy）；节点上任意时刻只有一条流水线。**【v1.1.2 扩展，P-1】构造 SSH 路径操作进入 `AwaitingConfirmation`，在确认前派发第二条变更操作**：断言第二条被 409 `MachineBusy` 拒绝、Deployment 目标阻塞、UI 呈现等待确认；确认/取消/跳过三个端点可用且不创建第二条未决操作；计划超时后互斥释放、操作以 `Failed` 终结 | A2、§4.3、§9.3 |
...
| T10 | **工件与 bundle 预算** | 超限 fixture（**【v1.1.2 扩展，P-6】四项预算各一**：单工件归档/上传态大小、解包后总大小、文件数、bundle 总量）：解析期/构建期即失败，错误码 `ArtifactTooLarge`/`BundleTooLarge`，不产生长时间传输 | S1、FR-6.8、§4.7 |
...
| T16 | **A→B→A 序列** | 单测：A→B→A 后 generation 递增、按代唯一行存在、回滚到中间代的引用可解析。**【v1.1.2 扩展，P-2】三台机器经 gen12→gen13→gen14 后执行 `POST /deployments/{id}/rollback` 回滚到 gen12**：断言**没有**目标被判 `Superseded`、每台机都执行重放并**各自**产生新代、未托管字段保留、各机新代内容等于 gen12 内容、Deployment 终态 `Succeeded`；反例断言：**非回滚** Deployment 指向低于当前代的目标时仍按 FR-10.6 判 `Superseded` | F6、ADR-2、FR-10.1/10.4/10.6、§7.2 |
...
| J 回滚 | FR-2.4、FR-9.6、**FR-7.5**、**FR-10.1/10.4/10.6（v1.1.2 补，P-2）**、§7.2 | 重放式回滚 + 保护引用集合 §10.4；**回滚型 Deployment 走"每机物化新代"分支，不被代一致性检查自我阻塞** |
...
| 确认窗口内互斥空缺（v1.1.2 新增） | 无 | T2 扩展 |
| 回滚 Deployment 自我阻塞（v1.1.2 新增） | 部分（J 只验证"能恢复"） | T16 扩展 |
...
| KM-12 安全建议若干 | token 熵取特定数值、永久保留所有快照、不备份 CA、删除机器后仍允许旧证书连接、Unknown 目标允许人工跳过 | 这些均未获得原 spec 的明确授权或验收依据，不能由评审直接宣布已确认。token 熵给出 ≥128-bit 的下界（安全属性而非具体取值）。**【v1.1.1 更正】"不备份 CA"由用户决定取代**：仍不新增内置产品导出动作（范围边界不变），人工备份/恢复/演练是必须运维契约（§2.2、§4.6、RD-7）。**【v1.1.2 更正，P-7】"删除后证书仍有效"与"跳过 Unknown"两项已由用户 2026-09-21 明确接受**（来源：KM-19 评论 `01a0c3e9-bec5-716d-93e4-46c8ddf897c3`），记为**用户已确认的取舍**并各带边界（§2.2）：前者以"删除即拒绝其证书"为最短路径、不引入 CRL/OCSP；后者要求审计记录 + UI 显式标出、不允许静默跳过、不计入成功。（历史引文保留：v1.1.1 曾把这两项写作"已接受风险"。） |
...
### 14.5 用户已授权的显式 FR 例外：FR-14.4 前端技术栈（KM-18 登记）

| 项目 | 内容 |
|---|---|
| 例外对象 | **FR-14.4**（Web UI SPA 技术栈）与 §3.5 技术栈表"前端"行 |
| 原表述 | `SPA 技术栈推荐 React + TypeScript + Vite；API 契约不得依赖前端框架` |
| 现表述 | `Svelte + TypeScript + Vite + shadcn-svelte（组件按页面需要引入）；API 契约不得依赖前端框架` |
| 技术栈决策来源 | 用户 2026-09-20 在 KM-15 定案（评论 `01a0bec5-3837-75bc-b322-1a6140fd8a83`：「把 shadcn-svelte 定位前端页面所使用库」） |
| 例外授权来源 | 用户 2026-09-21 在 KM-19 确认（评论 `01a0c3e9-bec5-716d-93e4-46c8ddf897c3`：「**用户已授权的显式 FR 例外，确定要用** Svelte + shadcn-svelte」） |
| 与哪条边界冲突 | §1.6「本版**不新增、不改写任何 FR**」。该边界是 v1.1.1 为 CA 备份决策自设的；用户已为前端栈单独授权例外，故本版在留痕后可改写 FR-14.4 |
| 例外范围（严格限定） | **只改技术栈表述**：不改 FR-14.4 的优先级（P0）与 spec 出处（§24）；不新增任何前端功能需求；不改 NFR-9（仍只要求"API 契约不依赖前端框架"）；不改 FR 计数与 §2.8 统计（95 条 / P0 94 / P1 1） |
| spec 侧留痕 | `agent-fleet-mvp-spec.md` §24 已同步改写并附修订记录（标注用户 2026-09-20 决定）；该处在 spec 中是**推荐**而非强制，改写不构成对 spec 的偏离（架构 §1.1 的"与 spec 冲突以 spec 为准并回写"规则因此未被触发） |
| 检查记录 | 现行文档中不再存在 React / Vue / Next.js 等其它框架的技术栈表述；历史文档 `agent-fleet-architecture-design.md`（v1.0）、`agent-fleet-architecture-v1.1.md`、`agent-fleet-architecture-v1.1.1.md` 保留原表述不改，作为历史记录（检查结果见 v1.1.2 supplement §3） |
...
11. **【新增】证书吊销机制**：完整 CRL/OCSP，或短证书 + 频繁续期；MVP 明确不做，只实现"**删除机器即拒绝其证书**"的最短路径（§2.2 用户已确认的取舍 (a)、§4.6）；
...
| # | 风险/开放问题 | 影响 | 缓解 |
|---|---|---|---|
| R1 | **已支持家族**（批次一 Codex / OMP / OpenCode / ZCode；批次二 Claude / DeepSeek Harness / Grok / Hermes，见 §2.2）的配置 schema/路径随版本漂移，适配器维护成本；批次二家族的运行时身份尚未实测确认 | 适配器失效 → 误报/漏报 drift；批次二接入前基线不明 | 逐家族 fixture 测试锁定行为（§32.2）；schema 版本协商挡住不兼容 agentd（§7.6）；**投影契约由适配器单点实现（ADR-1），改动面收在一个包内**；批次二家族按 [Agent 支持矩阵](agent-support-matrix.md) 的"接入前确认"逐项验证身份与能力，**未确认不得进入实现**（v1.1.2，P-9） |
...
| R7 | **无证书吊销（无 CRL/OCSP）：删除机器只做到"删除即拒绝"** | 删除**之前**已被复制或借用的证书材料不因删除而失效；删除不是通用安全措施 | **用户已确认的取舍 (a)**（2026-09-21）：控制面必须**拒绝已删除机器证书的后续 `Connect`**（§4.6、FR-11.6）；不引入完整 PKI/HSM/CRL/OCSP；UI 与运维口径标注「**当前限制：无证书吊销**」；安全需求走 §4.6 分支三（重建信任）（§2.2、§11） |
...
> **统计口径说明【契约】**：上述数字由脚本对 §2.4 表格逐行机械统计得到（v1.1 新增 25 条，v1.0 为 70 条），并与 §2.8 完全一致。v1.0 此处写"55 条"而表格实为 70 条，属于正文统计文字未随表格同步；本版以表格为准，统计验证记录见 `architecture-revision-resolution.md` §6.1（其口径更正见 v1.1.2 supplement）。**【v1.1.2，P-3/P-8】统计断言由 `docs/tools/verify-architecture-stats.py` 校验**：(1) §2.8 每行 15 列合计 == 所印合计；(2) 逐列满足 v1.0 + 新增 = v1.1；(3) §2.4 机械统计 = 95 条（P0 94 / P1 1），且 §2.8 / §18 / 附录 A / 附录 B 四处一致；(4) 全篇不存在作为本版数字的 96/70/+26。**该段数字不得手写，必须由统计脚本生成后核对。**
...
v1.1.2（KM-18）在同一口径上再补三处边界修正：**确认窗口不再是互斥的空档**（`AwaitingConfirmation` 计入未决操作与唯一索引，P-1）；**回滚 Deployment 不再被自己的代一致性检查阻塞**（回滚走"每机物化新代"分支，P-2）；**未获用户确认的取舍不再被写成"已接受"**（两项已由用户 2026-09-21 确认并带边界，P-7；其余一致性缺陷按 P-3～P-15 逐项更正——错误码补入 `RollbackUnsupported` 与非错误码枚举的显式归类、条件置位方列补齐、统计差值行与"96 项"残留更正、术语与引用卫生统一）。这些改动**只改文档契约表述，不改 FR 编号与统计口径**（95 条，P0 94 / P1 1），也不新增产品功能。
...
**本版状态**：**v1.1.2 文档修订版，待用户最终验收。** 本文档不构成评审通过，也不构成任何软件测试结论——本版未运行产品代码、未执行攻击、未做性能实测；文中所有规模与性能数字均为转引或算术推演，容量结论已按 §12.1 标注为待验证假设。v1.1.2 的增量是：**KM-17 独立复核报告的全部发现（P-1～P-15）已逐项闭环**（处置表与未验证项清单见 `architecture-revision-resolution-v1.1.2-supplement.md`）；前端技术栈落地为 Svelte + TypeScript + Vite + shadcn-svelte（§14.5 的显式 FR 例外）；两项取舍与八家族分批按用户 2026-09-21 裁决落地（§2.2）。**独立复核结论的验收与最终验收均由用户确认；CA 备份/恢复演练（§13.5 DR-1～DR-8）尚未执行**，不得据此宣称 CA 已可恢复；容量假设 H1–H3、测试项 T1–T22 与产品实现本身同样未执行、未验证。
...
**未改动项声明（v1.1.2 更正，P-10）**：§2.8 统计口径、§13.1–13.4 的 T1–T22、§14.1/14.2/14.3 其它 RD、§15 追溯表、§16 演进路线与 v1.1 逐字一致（除为指向新增小节而必需的极少数措辞外）。**但原文"§2.4 全部 FR（含编号与计数）……均与 v1.1 逐字一致"这一声明与文件实际状态不符**：文件头记载的「2026-09-20 Agent 范围补充」已改写 **FR-4.2**，并触动 §2.2（Agent 生态行）与 §5.4（补充契约）；此外 §3.6、§6.1、R1 当时仍是三家族表述，未随八家族同步（P-9）。该补充经用户截图授权，**不构成范围违规**；v1.1.2 已更正记账（§1.6、§1.7）并在下方附录 B 列出全部触及位置。

---

## 附录 B：v1.1.2 触及位置索引（供复核与验收逐条核对）

| # | 位置 | 类型 | 摘要 | 对应发现 |
|---|---|---|---|---|
| 1 | 文首 + 文档头表 + §1.3 + §1.5 + §1.6 + §1.7 | 新增/修改 | v1.1.2 增量说明；版本/日期/依据/执行人/状态；术语表互斥集合补 `AwaitingConfirmation`；不变量命名更正；记账范围更正；新增 §1.7 全量变更清单（43 项） | P-1、P-10、P-12、用户裁决 |
| 2 | §2.2 | 修改 | Agent 生态分批；"已接受风险"标题改为"明确不防御项与残余风险"；新增"用户已确认的取舍"（含 (a)(b) 边界） | P-7、用户裁决 |
| 3 | §2.4 | 修改 | FR-1.10、FR-10.1、FR-10.4、FR-10.6、FR-11.6、FR-13.9、FR-14.4、FR-15.2 | P-1、P-2、P-4、P-7、P-11、前端决策 |
| 4 | §2.8 | 修改 | FR-11 新增列 `+2`→`+3`；"96 项"→95；新增统计断言与校验脚本 | P-3、P-8 |
| 5 | §3.1 / §3.4 / §3.5 / §3.6 | 修改 | 不变量引用更正；确认窗口与回滚分支标注；前端栈；家族包一般化与批次归属 | P-12、P-1、P-2、P-9、前端决策 |
| 6 | §4.2 | 改写 | 条件置位方写入纪律；写入规则表补"置位方"列；末行"操作终态"改"操作处于…时" | P-5、P-14 |
| 7 | §4.3 / §4.9 | 修改 | 唯一索引谓词补 `'AwaitingConfirmation'`；新增第 6 条"互斥的合法例外"；确认窗口并发处置 | P-1 |
| 8 | §4.4 / §7.2 | 修改 | 回滚型 Deployment 物化每机新代、"内容代"语义、`RollbackUnsupported` 分支 | P-2 |
| 9 | §4.6 / §11 / §16 / §17 R7 | 改写 | 删除即拒绝其证书；"当前限制：无证书吊销"；安全需求走 §4.6 分支三 | P-7、用户裁决 |
| 10 | §4.7 / §7.4 | 修改 | 工件预算补第四项（归档/上传态大小，默认值待负载验证） | P-6 |
| 11 | §5.2 / §5.3 / §9.7 / FR-13.9 | 修改 | 自动 apply 措辞更正；"可变步骤 5–12"与"可中断阶段 5–10 / 不可中断阶段 11–13"分离定义 | P-11、P-14 |
| 12 | §5.4 / §6.1 | 修改 | 家族接入批次；AgentProfile 家族键；Deployment 增 `rollbackOf` | 用户裁决、P-9、P-2 |
| 13 | §6.4 | 修改 | Reconcile 错误 14→15 类（补 `RollbackUnsupported`）；非错误码枚举显式归类；确认窗口互斥闭环 | P-1、P-4 |
| 14 | §7.3 / §9.6 | 修改 | 跳过的审计与 UI 边界（操作者/时间/目标/原因；不允许静默跳过；不改变实际状态；不计入成功） | P-7、用户裁决 |
| 15 | §8.1 | 修改 | 三个互斥合法例外端点 | P-1 |
| 16 | §9.3 | 新增 | 确认窗口内的并发处置规则 5 条 | P-1 |
| 17 | §13.1 | 修改 | T2、T10、T16 扩展断言（P-1 / P-6 / P-2 的验收场景） | P-1、P-2、P-6 |
| 18 | §13.3 / §13.4 | 修改 | J 行补 FR-10.x；反例表 19 → 21 行 | P-2 |
| 19 | §14.4 / §14.5 | 修改/新增 | 两项"已接受风险"改为"用户已确认的取舍"；新增显式 FR 例外登记 | P-7、前端决策 |
| 20 | §17 R1 | 修改 | 八家族分批 + 批次二身份待实测 | P-9、用户裁决 |
| 21 | §18 | 修改 | 统计断言与校验脚本；v1.1.2 边界修正段；本版状态改为"待用户最终验收" | P-3、用户裁决 |
| 22 | 附录 A（更正）/ 附录 B（新增） | 修改/新增 | 更正"§2.4 全部 FR 逐字一致"的失实声明；新增本索引 | P-10 |
| 23 | 关联文件 spec §24 | 修改 | 前端栈改写 + 修订记录（2026-09-20 用户决定） | 前端决策 |

**未改动项声明（v1.1.2）**：§2.4 的 FR **编号与优先级**、§2.8 统计口径与合计（95 / P0 94 / P1 1）、§2.5 NFR（含 NFR-9 原文）、§3.1 不变量表条目集合、§4.6 CA 备份/恢复契约九条、§4.7 前三项预算的默认值、§13.5 演练项 DR-1～DR-8、§14.1/14.2/14.3 决策、§15 追溯表、§16 其余演进项、§17 其余风险项与 O1–O4，均**未改动**；历史文档 `agent-fleet-architecture-v1.1.1.md` 原件保留不改。全部改动仅限文档契约表述，不涉及产品实现、部署或测试执行。
